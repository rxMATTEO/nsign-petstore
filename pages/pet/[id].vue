...
function onModalClose(){
  ...
  rememberPetStore.setRememberPet(_rememberPet);
}
</script>
  ...
  <div v-if="pet.id">
    <div class="flex gap-5 justify-center items-center">
      ...
      <div class="absolute right-0 w-1/2 top-0">
        <ItemsModal :quantity="10" :items="sameCategoryPets" @item-click="(item: Pet) => {
          clickedItem = item;
      ...
      <template #default>
        <div class="flex flex-col">
          <div>
            ...
            <input class="border-black border-2" type="text" v-model="rememberPet" />
          </div>
          <button class="mt-5 bg-blue-400 rounded-xl p-3" @click="onModalClose">Запомнить просмотр</button>
        </div>
      </template>
    </FullScreenModal>
  </div>
